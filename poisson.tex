...
nejkratší vzdálenost. Takto můžeme zavést definici úsečky v~jakémkoli pseudoprostoru. Přesnou definici křivky si nechme na později.
...
\(|\vect{u}|^2 - 2 |\vect{u}| \cdot |\vect{v}| + |\vect{v}|^2 \leq |\vect{u} + \vect{v}|^2 \leq
|\vect{u}|^2 + 2 |\vect{u}| \cdot |\vect{v}| + |\vect{v}|^2\), neboli
 \((|\vect{u}| - |\vect{v}|)^2 \leq |\vect{u} + \vect{v}|^2 \leq
|(\vect{u}|+ |\vect{v}|)^2\) a~tedy
 \(||\vect{u}| - |\vect{v}|| \leq |\vect{u} + \vect{v}| \leq
|\vect{u}|+ |\vect{v}|\).

Vidíme, že je splněna podmínka \eqref{eq:pseudometricky_prostor_definice_3}, kterou jsme kladli na metriku v~pseudometrickém prostoru. Splnění ostatních podmínek nebudeme rozebírat, čtenář si je může triviálně ověřit sám. Dále vidíme, že aby platilo \(|\vect{u} + \vect{v}| = |\vect{u}|+ |\vect{v}|\), tak
je nutné, aby \(\unitvect{u} = \unitvect{v}\), tedy aby oba vektory měli stejný směr.


\(n\)-rozměrný Euklidovský prostor \(E_n\) je metrický prostor, jehož prvky jsou body, a v němž platí Pythagorova věta. Každému bodu tohoto prostoru
je možné přiřadit unikátní \(n\)-tici reálných čísel, takovémuto přiřazení říkáme souřadnicový systém.

Jedním ze způsobů, jak v~Euklidovském prostoru zavést souřadnicový systém, je zvolit v~něm bod odpovídající počátku souřadnicového systému a~z něj vést \(n\) vzájemně kolmých souřadnicových os. Souřadnice jekéhokoli
bodu je pak \(n\)-tice čísel odpovídajících orientovaným vzdálenostem průmětu bodu do jednotlivých souřadnicových os. Ve rovině je zvykem osy a složky souřadnic označovat \(x\) a \(y\), v prostoru \(x\), \(y\) a \(z\).
...
Máme-li dva body, pak jejich rozdíl je v~Euklidovském prostoru vektorem. Na obrázku \ref{img:kartezsky_system_souradnic} máme zakreslen vektor \(\overrightarrow{AB} = B - A\). Vektor představuje veličinu, která
má směr a~orientaci. Vektor je v~\(n\)-rozměrném prostoru určen \(n\) složkami. Ty odpovídají jeho průmětům do souřadnicových os. Vektor \(\overrightarrow{AB}\) má tedy složky \(\overrightarrow{AB} = B - A = (3-2, 4-1) = (1, 3)\).





V~rovině (dvojrozměrném prostoru) metriku odvodíme podle Pythagorovy věty. Mějme body \(A\) a \(B\). Jejich souřadnice označme \([A_x, A_y]\) a~\([B_x, B_y]\). Pak délku úsečky \(AB\) můžeme vypočítat jeko délku přepony pravoúhlého trojúhelníka, jehož odvěsny mají délku \(B_x - A_x\) a \(B_y - A_y\). Délka přepony a~tedy i~metrika je \(d(A, B) = \sqrt{(B_x - A_x)^2 + (B_y - A_y)^2}\).


Pokud bychom měli trojrozměrný prostor,
pak bychom metriku spočítali jako délku tělesové úhlopříčky hranolu o~stranách \(B_x - A_x\), \(B_y - A_y\) a~\(B_z - A_z\), tedy \(d(A, B) = \sqrt{(B_x - A_x)^2 + (B_y - A_y)^2 + (B_z - A_z)^2}\).

Tento postup můžeme zobecnit. V~obecném \(n\)-rozměrném prostoru je metrika určena vztahem \eqref{eq:euklidovsky_prostor_metrika}. Složky souřadnic v~tomto případě označujeme alternativně pomocí indexů, tedy \([u_1, u_2, ..., u_n]\).

Platnost tohoto vztahu dokážeme matematickou indukcí. Pro \(n = 1\) vztah přejde v~\(d(u, v) = \sqrt{(v_1 - u_1)^2} = |v_1 - u_1|\) a~evidentně platí - určuje vzdálenost bodů na přímce. Teď si představme, že máme \(n-1\) rozměrný
podprostor a~metrika průmětů bodů \(u\) a~\(v\) do tohoto podprostoru je \(\sqrt{\sum_{i=1}^{n-1} (v_i - u_i)^2}\). To je tedy délka úsečky spojující tyto průměty a~tvoří jednu stranu trojúhelníka. Druhá strana je dlouhá \(v_n - u_n\) a~je na ni kolmá (to si dokážeme později). Metrika je tedy určena jako délka přepony pravoúhlého trojúhelníka podle vztahu \eqref{eq:euklidovsky_prostor_metrika_odvozeni}. Vztah \eqref{eq:euklidovsky_prostor_metrika} platí pro \(n = 1\) a~pokud
platí pro \(n-1\), pak platí i~pro \(n\). Platí proto pro všechna přirozená čísla \(n \in \mathbb{N}\).

\begin{equation}
\label{eq:euklidovsky_prostor_metrika_odvozeni}
\begin{split}
d(u, v) = \sqrt{\left(\sqrt{\sum_{i=1}^{n-1} (v_i - u_i)^2} \right)^2 + (v_n - u_n)^2} = \\
\sqrt{\sum_{i=1}^{n-1} (v_i - u_i)^2 + (v_n - u_n)^2} = \\
...
Kartézský systém souřadnic není jediný možný.
