...
Uvažujme například souřadný systém definovaný transformací na kartézský souřadný systém vztahy:

\begin{equation}
\begin{split}
x = 2 \cdot r \cdot \cos \alpha \\
y = 3 \cdot r \cdot \sin \alpha
\end{split}
\end{equation}

Opačná transformace je možná vztahy

\begin{equation}
\begin{split}
r = \sqrt{\frac{x^2}{4} + \frac{y^2}{9}} \\
\alpha = \mathrm{arctg} \frac{2y}{3x}
\end{split}
\end{equation}

Dle předchozího zápisu jsou souřadnice \(P = [x, y]\), \(Q = [r, \alpha]\) a~transformační funkce:

\begin{equation}
\begin{split}
x = p_1(r, \alpha) = 2 \cdot r \cdot \cos \alpha \\
y = p_2(r, \alpha) = 3 \cdot r \cdot \sin \alpha \\
r = q_1(x, y) = \sqrt{\frac{x^2}{4} + \frac{y^2}{9}} \\
\alpha = q_2(x, y) = \mathrm{arctg} \frac{2y}{3x}
\end{split}
\end{equation}

V~našem příkladě mějme skalární funkci \(\varphi(r, \alpha) = r^2 \cdot \sin 2 \alpha\). Dosadíme-li za souřadnice \(r, \alpha\), získáme \(\phi(x, y) = (\frac{x^2}{4} + \frac{y^2}{9}) \cdot \sin \left(2 \cdot \mathrm{arctg} \frac{2y}{3x} \right)\).

Máme tedy funkci \(\varphi\) v~souřadnicích \(Q\) a~funkci \(\phi\) v~souřadnicích \(P\). Vyjádřeme první derivaci funkce \(\phi\) podle souřadnice \(P_i\). Obecně platí:
...
Uvažujme napřed rozdíl souřadnic dvou nekonečně blízkých bodů.

Ten může vzniknout například tak, že derivujeme souřadnice dráhy podle nezávislé proměnné, například času. Příkladem je tedy rychlost nebo zrychlení.
