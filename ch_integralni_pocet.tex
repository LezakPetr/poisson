...
\begin{equation}
\underline{\int}_a^b = \sum_{i} (x_{i+1} - x_i) \cdot \min(\{\mathrm{f}(x) | x_i \leq x \leq x_{i+1}\})
\end{equation}
...
\begin{equation}
\overline{\int}_a^b = \sum_{i} (x_{i+1} - x_i) \cdot \max(\{\mathrm{f}(x) | x_i \leq x \leq x_{i+1}\})
\end{equation}
...
Dělení intervalu nemusí být rovnoměrné. Označme toto dělení \(d\) a~nejdelší interval tohoto dělení \(\delta(d) = \max(x_{i+1} - x_i)\). Pak můžeme spočítat limity dolních a~horních mezí. Povšimněme si, že dělení intervalu není určeno jednoznačně a~my v~limitách bereme v~potaz nejhorší případ:

\begin{equation}
\lim_{\delta_m \to 0} \min \left(\left\{\underline{\int}_a^b \right | \delta(d) \leq \delta_m\}\right)
\end{equation}

\begin{equation}
\lim_{\delta_m \to 0} \max \left(\left\{\overline{\int}_a^b \right | \delta(d) \leq \delta_m\}\right)
\end{equation}


Pokud horní a~dolní mez mají stejnou limitu, pak je tato limita rovna integrálu.

\begin{equation}
\int_a^b = \lim_{\delta_m \to 0} \min \left(\left\{\underline{\int}_a^b \right | \delta(d) \leq \delta_m\}\right)
 = \lim_{\delta_m \to 0} \max \left(\left\{\overline{\int}_a^b \right | \delta(d) \leq \delta_m\}\right)
\end{equation}
