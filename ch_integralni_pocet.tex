...
Mějme funkci s~více parametry \(\mathrm{f}(x, y, ...)\), kterou budeme integrovat obecně na hyperkvádru se stranamy \(<a_x; b_x>\), \(<a_y; b_y>\) atd. Tyto strany rozdělíme na konečný počet intervalů \(<x_0, x_1>\), \(<x_1, x_2>\), ...,  \(<y_0, y_1>\), \(<y_1, y_2>\) atd. Tím vznikne mřížka dělící integrovaný hyperkvádr na malé hyperkvádry. Dolní mez integrálu je pak dána vztahem

\begin{equation}
\underline{\int}_a^b \mathrm{f}(x, y, ...) \cdot \mathrm{d}V = \sum_{i, j, ...} (x_{i+1} - x_i) \cdot (y_{j+1} - y_j) \cdot ... \cdot \min_{i, j, ..}(\mathrm{f})
\end{equation}
...
\begin{equation}
\overline{\int}_a^b \mathrm{f}(x, y, ...) \cdot \mathrm{d}V = \sum_{i, j, ...} (x_{i+1} - x_i) \cdot (y_{j+1} - y_j) \cdot ... \cdot \max_{i, j, ..}(\mathrm{f})
\end{equation}
