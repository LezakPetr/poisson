...
?-	make_test_natural_numbers(Values),
	print_validated_formula(
		...
		declare([variable(A, 'a', Values), variable(B, 'b', Values), variable(N, 'n', Values)],
			equal([
				(A*B)^(N+1),
				(A*B)^N * A*B,
				A^N * B^N * A*B,
...
?-	make_test_numbers(Numbers),
	...
	print_validated_formula(
		'division_definition',
		declare([variable(A, 'a', Numbers), variable(B, 'b', NonzeroNumbers), variable(C, 'c', Numbers)],
			equiv(
				equal([C, A / B]),
				equal([B * C, A])
			)
...
?-	Numbers = [1, 2, 3, 4, 6],
	print_validated_formula(
		...
		declare([
			variable(A, 'a', Numbers), variable(B, 'b', Numbers), variable(C, 'c', Numbers),
			variable(D, 'd', Numbers), variable(E, 'e', Numbers), variable(F, 'f', Numbers)
		],
			proof([
				equal([C, A / B]),
				equal([F, D / E])
...
?-	make_test_nonzero_numbers(Numbers),
	print_validated_formula(
		'rational_reduction',
		declare([variable(A, 'a', Numbers), variable(B, 'b', Numbers), variable(C, 'c', Numbers)],
			equal([
				A / B,
				(A / B) * 1,
				(A / B) * (C / C),
...
?-	make_test_numbers(Numbers),
	...
	print_validated_formula(
		...
		declare([plus_minus(PM), variable(A, 'a', Numbers), variable(B, 'b', Numbers), variable(C, 'c', NonzeroNumbers)],
			equal([
				plus_minus(A, B, PM) / C,
...
?-	make_test_numbers(Numbers),
	...
	print_validated_formula(
		'sum_of_rationals',
		declare([
			plus_minus(PM), variable(A, 'a', Numbers), variable(B, 'b', NonzeroNumbers),
			variable(C, 'c', Numbers), variable(D, 'd', NonzeroNumbers)
		],
			equal([
...
?-	make_test_numbers(Numbers),
	...
	print_validated_formula(
		...
		declare([
			variable(A, 'a', Numbers), variable(B, 'b', NonzeroNumbers),
...
?-	make_test_nonzero_integers(TestBases),
	...
	print_validated_formula(
		...
		declare([variable(A, 'a', TestBases), variable(B, 'b', TestExponents)],
			impl(
				not_equal(A, 0),
				equal([
					1,
...
?-	make_test_nonzero_integers(TestBases),
	print_validated_formula(
		...
		declare([variable(A, 'a', TestBases)],
			impl(
				not_equal(A, 0),
				equal([A^(-1), 1 / A])
			)
...
?-	make_test_numbers(Numbers),
	...
	print_validated_formula(
		'order_rationals_1',
		declare([variable(A, 'a', Numbers), variable(B, 'b', NonzeroNumbers)],
			equal(A / B, (A * B) / (B * B))
		)
	).
\end{prolog}
...
?-	make_test_positive_numbers(Bases),
	...
	print_validated_formula(
		'sqrt_rational_proof',
		declare([variable(A, 'a', Bases), variable(B, 'b', Exponents)],
			equal([
				A,
				A^1,
				A^(B / B),
...
\end{prolog}
\eimg{pow_function_1}{Graf mocniny o~základu \(2\)}
...
\end{prolog}
\eimg{pow_function_2}{Graf mocniny o~základu \(\frac{1}{2}\)}
...
?-	make_test_positive_numbers(Numbers),
	...
	print_validated_formula(
		'log_product_proof',
		declare([variable(A, 'a', Bases), variable(B, 'b', Numbers), variable(C, 'c', Numbers)],
			proof(
			[],
			[
				equal(B * C, B * C),
...
?-	make_test_log_bases(Bases),
	...
	print_validated_formula(
		'log_power_proof',
		declare([variable(A, 'a', Bases), variable(B, 'b', Numbers), variable(C, 'c', Exponents)],
			proof(
			[],
			[
				equal(B^C, B^C),
...
?-	make_test_log_bases(Bases),
	...
	print_validated_formula(
		'log_base_proof',
		declare([variable(A, 'a', Bases), variable(B, 'b', Numbers), variable(C, 'c', Bases)],
			proof(
			[],
			[
				equal(B, B),
