...
Operace součtu zůstává definovaná pomocí tohoto pořadí a~rovnic~\eqref{eq:natural_numbers_comutativity} a~\eqref{eq:natural_numbers_asociativity}.
...
?-	make_test_numbers(Values),
	print_validated_formula(
		...
		declare_variable(A, 'a', Values,
			impl(
				num_not_equal(A, 0, 1e-12),
				num_equal([A^0, 1], 1e-12)
			)
		)
	).
\end{prolog}
					...
					declare_variable(D, 'd', Numbers,
						declare_variable(E, 'e', Numbers,
							declare_variable(F, 'f', Numbers,
								proof([
									num_equal([C, A / B], 1e-12),
									num_equal([F, D / E], 1e-12)
								],
								[
									num_equal([A, B * C], 1e-12),
									num_equal([D, E * F], 1e-12),
									num_equal([A * D, par(B * C) * par(E * F), par(B * E) * par(C * F)], 1e-12),
									num_equal([(C * F), (A / B) * (D / E), (A * D) / (B * E)], 1e-12)
								])
							)
						)
					)
