...
\end{prolog}
\eeq{ap_derivative_general_power_proof}
...
?-	print_validated_formula(
		"ap_euler_number_1",
		equal(
			lim(DX, "\\mathrm{d}x", real, 0, (e^DX - 1) / DX),
			1
		)
	).
\end{prolog}
\eeq{ap_euler_number_1}
...
?-	print_validated_formula(
		...
		proof(
			...
			[
				equal(
					lim(DX, "\\mathrm{d}x", real, 0, e^DX - 1),
					lim(DX, "\\mathrm{d}x", real, 0, DX)					
				),
				equal(
					lim(DX, "\\mathrm{d}x", real, 0, e^DX),
					lim(DX, "\\mathrm{d}x", real, 0, 1 + DX)					
				),
				equal(
					lim(DX, "\\mathrm{d}x", real, 0, e),
					lim(DX, "\\mathrm{d}x", real, 0, sqrt(DX, 1 + DX))					
				)
			]
		)
	).
\end{prolog}
\eeq{ap_euler_number_2}
...
\end{prolog}
\eeq{ap_derivative_ln_exp}


\begin{prolog}
?-	make_test_real_numbers(Numbers),
	print_validated_formula(
		"ap_derivative_exp_ln",
		declare(
			[
				variable(X, "x", Numbers)
			],
...
?-	make_test_real_numbers(Numbers),
	print_validated_formula(
		"ap_derivative_general_power_by_exp",
		declare(
			[
				variable(X, "x", Numbers),
...
\end{prolog}
\eeq{ap_derivative_general_power_by_exp}
...
\eeq{derivative_ln_proof_1}

a~použitím vztahu~\eqref{eq:derivace_inverzni_funkce} získáme

\eeq{derivative_ln_proof_2}

\subsubsection{Derivace reálné mocniny}

Dříve jsme dokázali vztah \(\frac{\partial}{\partial x} x^n = n \cdot x^{n-1}\) pro \(x \in \mathbb{R}\) a~\(n \in \mathbb{N}\). Nyní dokážeme, že tento vztah platí i~pro reálné exponenty. Protože platí
...
?-	make_test_real_numbers(Numbers),
	print_validated_multiformula(
		...
		[
			formula(
				"derivative_power_by_real_proof_1",
				impl(
					greater_than(X, 0),
					equal([
						X^R,
			...
			formula(
				"derivative_power_by_real_proof_2",
				impl(
					greater_than(X, 0),
					equal([
						derivative(X, X^R),
						derivative(X, e^(R * ln(X))),
						e^(R * ln(X)) * R * (1 / X),
						X^R * R * (1 / X),
						R * X^(R - 1)
					])
...
\eeq{derivative_power_by_real_proof_1}

tak proto platí

\eeq{derivative_power_by_real_proof_2}
...
\begin{equation}
\frac{\partial}{\partial x} \left(e^{2x+1} + 3x \right)^3 = ...
\end{equation}
...
\begin{equation}
... = 3 \cdot \left(e^{2x+1} + 3x \right)^2 \cdot \frac{\partial}{\partial x} \left(e^{2x+1} + 3x \right) = ...
\end{equation}

Využijeme pravidlo pro derivaci součtu:

\begin{equation}
... = 3 \cdot \left(e^{2x+1} + 3x \right)^2 \cdot \left(\frac{\partial}{\partial x} e^{2x+1} + \frac{\partial}{\partial x} 3x \right) = ...
\end{equation}
...
\begin{equation}
... = 3 \cdot \left(e^{2x+1} + 3x \right)^2 \cdot \left(e^{2x+1} \cdot \frac{\partial}{\partial x} \left(2x+1\right) + 3 \cdot \frac{\partial}{\partial x} x \right) = ...
\end{equation}

Na první člen použijeme pravidlo pro derivaci součtu. Derivace druhého členu \(x\) je 1:

\begin{equation}
... = 3 \cdot \left(e^{2x+1} + 3x \right)^2 \cdot \left(e^{2x+1} \cdot \left(\frac{\partial}{\partial x} 2x + \frac{\partial}{\partial x} 1\right) + 3 \right) = ...
\end{equation}
...
?-	make_test_real_numbers(Numbers),
	...
	print_validated_formula(
		...
		declare(
			...
			equal([
				derivative(Y * X, apply(F, [FX, FY], [X, Y])),
				linebreak,
				lim(DX, "\\mathrm{d}x", real(1e-5), 0,
					(
						lim(DY, "\\mathrm{d}y", real(1e-5), 0,
							(apply(F, [FX, FY], [X + DX, Y + DY]) - apply(F, [FX, FY], [X + DX, Y])) / DY
						) -
						lim(DY, "\\mathrm{d}y", real(1e-5), 0,
							(apply(F, [FX, FY], [X, Y + DY]) - apply(F, [FX, FY], [X, Y])) / DY
						)
					) / DX
				),
				linebreak,
				lim([DX, DY], ["\\mathrm{d}x", "\\mathrm{d}y"], [real(1e-5), real(1e-5)], [0, 0],
					(
						nopar(apply(F, [FX, FY], [X + DX, Y + DY]) - apply(F, [FX, FY], [X + DX, Y]))
						- apply(F, [FX, FY], [X, Y + DY]) + apply(F, [FX, FY], [X, Y])
					) / (DX * DY) 
				),						
				linebreak,
				lim(DY, "\\mathrm{d}y", real(1e-5), 0,
					(
						lim(DX, "\\mathrm{d}x", real(1e-5), 0,
							(apply(F, [FX, FY], [X + DX, Y + DY]) - apply(F, [FX, FY], [X, Y + DY])) / DX
						) -
						lim(DX, "\\mathrm{d}x", real(1e-5), 0,
							(apply(F, [FX, FY], [X + DX, Y]) - apply(F, [FX, FY], [X, Y])) / DX
						)
					) / DY
				),
				linebreak,
				derivative(X * Y, apply(F, [FX, FY], [X, Y]))
			])
		)
	).
\end{prolog}
\eeq{derivative_order_equal_proof}
