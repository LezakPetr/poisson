...
?-	make_test_real_numbers(Numbers),
	...
	print_validated_multiformula(
		[
			...
			substitution(URT, "u(t)", imag_part(UR * e^(imag * Omega * T))),
...
Povšimněme si, že rovnice~\eqref{eq:serial_resonance_coil_img} připomíná Ohmův zákon s~odporem \(L \cdot \imag \cdot \omega\). Tomuto \uv{komplexnímu odporu} se říká impedance, rovnici~\eqref{eq:serial_resonance_coil_img} proto můžeme přepsat do tvaru
...
Opět můžeme zavést impedanci kondenzátoru a~rovnici~\eqref{eq:serial_resonance_capacitance_img} přepsat do tvaru
